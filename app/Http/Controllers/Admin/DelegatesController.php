...
class DelegatesController extends Controller
{
    ...
    public function update($id, CustomerEditRequest $request)
    {
      try {
        $com_code = auth()->user()->com_code;
        $data = get_cols_where_row(new Customer(), array("id", "account_number", "delegate_code"), array("id" => $id, "com_code" => $com_code));
        if (empty($data)) {
        ...
        if ($flag) {
          ...
          $flag = update(new Account(), $data_to_update_account, array('account_number' => $data['account_number'], 'other_table_FK' => $data['delegate_code'], 'com_code' => $com_code, 'account_type' => 3));
        }
